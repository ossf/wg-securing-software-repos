...
In this example, as in the last page, we've used PyPI's visual styling with blue hyperlinks and an external link icon next to each link. Both the icon color and font can be changed to match existing styles. The use of an "open in a new tab" icon is optional.
...
* We recommend that there be a visual divider or way for the user to distinguish when maintainer provided content (e.g. the project description) ends and index-controlled content (e.g. security information) begins. This divider line follows the existing style of the site.
